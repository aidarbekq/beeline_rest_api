...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    async def run_name(self, name):
        print(f'running {name}...')

    async def send_my_headers(self):
        self.send_response(200)
        self.send_header('content-type', 'application/json')
        self.end_headers()

    async def only_json_response(self, content_type):
    ...
    async def received_response(self, data):  # эта функция возвращает ответ на POST метод
        await self.send_my_headers()
    ...
    async def do_POST(self):
        ...
        if content_type != 'application/json':  # проверка типа на JSON
            await self.only_json_response(content_type)
        ...
        # если проходит проверку, выполняется следующий код:
            ...
            await self.received_response(post_data)
